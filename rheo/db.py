...
def compare_experiments(
    ...
    *,
    ...
    fig = make_overlay_figure(frames, metas, metric=metric)
    return {"summary": pd.DataFrame(rows), "figure": fig, "metas": metas}


def make_overlay_figure(frames: dict[int, pd.DataFrame], metas: dict[int, dict[str, Any]], metric: str = "shear_stress_pa") -> plt.Figure:
    fig, ax = plt.subplots(figsize=(8.8, 5.6))
    cmap = plt.cm.get_cmap("tab10")
    for idx, (eid, df) in enumerate(frames.items()):
        color = cmap(idx % 10)
        name = metas[eid].get("name") or str(eid)
        grouped = _segment_data(df)
        for seg_name in ["up", "down", "all"]:
            if seg_name not in grouped:
                continue
            part = grouped[seg_name]
            if not {"shear_rate_1_s", metric}.issubset(part.columns):
                continue
            pdata = part[["shear_rate_1_s", metric]].dropna()
            pdata = pdata[(pdata["shear_rate_1_s"] > 0) & (pdata[metric] > 0)]
            if pdata.empty:
                continue
            marker = SEGMENT_MARKERS.get(seg_name, "o")
            ...
            _maybe_add_log_fit(
                ax,
                pdata["shear_rate_1_s"].to_numpy(dtype=float),
                pdata[metric].to_numpy(dtype=float),
                color,
                f"{name} {seg_name} fit",
            )
    _set_log_axes(ax, "Shear rate γ̇ (1/s)", PLOT_LABELS.get(metric, metric))
